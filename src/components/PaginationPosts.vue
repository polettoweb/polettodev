<template>
  <div class="flex justify-between text-xl items-center">
    <g-link :to="previousPage" :class="['pagination',{ 'cursor-not-allowed': !showPreviousPage }]">&larr; Prev</g-link>
    <div class="text-base">Page {{ currentPage }} of {{ totalPages }}</div>
    <g-link :to="nextPage" :class="['pagination',{ 'cursor-not-allowed': !showNextPage }]">Next &rarr;</g-link>
  </div>
</template>
